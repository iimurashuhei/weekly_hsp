...
# （3）Roisman's Approach（主効果モデルと交互作用モデルのF比を参照するため）
  * その他の時点の結果については、Rコードを確認ください。    
  * RoS testはRoismanのappを使用します：[https://www.yourpersonality.net/interaction/ros.pl]
...
![RoS on X testing](C:\Users\Iimura Shuhei\Documents\weekly_hsp\RoStest_T2.png)

## 3-3. 3時点目の分析
```{r roismant3, include = TRUE}
#pequodで分析版
...
# （4）時点ごとのWidaman's Approach
  * RMarkdown内で分析に使用した計算コードをもとに、数式を表現してくれるパッケージがある！⇒ [https://bookdown.org/yihui/rmarkdown-cookbook/equatiomatic.html]  
  * 論文執筆の際、Methodで数式を例示するために役に立ちそう！！
...
![HowToCreateWidamanFigure](C:\Users\Iimura Shuhei\Documents\weekly_hsp\HowToCreateWidamanFigure.png)

## 1時点目の作図
```{r weekoneplot, include = TRUE}
...
![WidamanPlot](C:\Users\Iimura Shuhei\Documents\weekly_hsp\WidamanPlot.png)
